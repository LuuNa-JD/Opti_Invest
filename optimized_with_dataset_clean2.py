...
# Fonction pour nettoyer les données et remplacer les coûts négatifs par leurs valeurs absolues
def nettoyer_donnees(actions):
    actions['price'] = actions['price'].abs()  # Remplacer les coûts négatifs par des valeurs positives
    actions_nettoyees = actions[actions['profit'] > 0]  # Garder seulement les actions avec un profit positif
...
# Nettoyer les deux datasets pour remplacer les coûts négatifs par des valeurs absolues
...
sauvegarder_resultats('resultats_dataset1_opti2.csv', meilleures_actions1, meilleur_profit1, total_investi1, budget_max)
sauvegarder_resultats('resultats_dataset2_opti2.csv', meilleures_actions2, meilleur_profit2, total_investi2, budget_max)
